...
interface SpaceTaskFocusContextProps {
  taskId: string;
  taskName: string;
}

export function SpaceTaskFocusContext({
  taskName,
}: SpaceTaskFocusContextProps) {
  return (
           ...
           <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                 <div className="p-3 rounded-xl bg-background border border-border/40 flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-bold text-foreground/60">Lead</span>
                 </div>
                 <div className="p-3 rounded-xl bg-background border border-border/40 flex items-center gap-3">
                    <Flag className="h-3.5 w-3.5 text-orange-500/50" />
                    <span className="text-[11px] font-bold text-foreground/60 uppercase tracking-widest">Urgent</span>
                 </div>
              </div>
           </div>
