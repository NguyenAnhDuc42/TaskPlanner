...
const MOCK_STATUSES = [
  ...
  { id: "s3", name: "Completed", color: "text-emerald-400", category: "completed" },
];
    ...
    <div className="flex-1 flex overflow-hidden h-full bg-transparent gap-1">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full bg-background border border-border overflow-hidden rounded-xl shadow-sm">
          ...
          <div className="flex items-center gap-3 px-6 h-12 flex-shrink-0 bg-background border-b border-border z-30">
            ...
            <div className="ml-8 flex items-center gap-1 h-full pt-1">
              {MOCK_TABS.map((tab) => (
                ...
                >
                   <tab.icon className={cn("h-3.5 w-3.5 opacity-40 group-hover:opacity-100", activeViewId === tab.id && "opacity-100")} />
                   {tab.name}
                </button>
              ))}
              <Button variant="ghost" size="icon" className="h-6 w-6 ml-2 text-muted-foreground/20 hover:text-primary transition-colors">
            ...
            <div className="ml-auto flex items-center gap-2">
               <div className="flex items-center gap-2 px-3 h-7 rounded-md bg-muted/30 border border-border">
                 ...
                 <span className="text-[10px] font-medium text-muted-foreground/30">Search view...</span>
               </div>
               <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground/40 hover:text-foreground border border-transparent hover:border-border">
                 <Settings2 className="h-3.5 w-3.5" />
               </Button>
               <Button 
                 variant="ghost" 
                 size="icon" 
                 className={cn("h-7 w-7 transition-colors rounded-sm", isPropsOpen ? "text-foreground bg-muted/60" : "text-muted-foreground/40 hover:text-foreground")}
                 onClick={() => setIsPropsOpen(!isPropsOpen)}
               >
                 {isPropsOpen ? <PanelRightClose className="h-3.5 w-3.5" /> : <PanelRight className="h-3.5 w-3.5" />}
               </Button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 px-6 h-9 flex-shrink-0 bg-background border-b border-border text-[10px] font-bold text-muted-foreground/60 uppercase tracking-tighter">
            <button className="flex items-center gap-1.5 hover:text-foreground transition-colors"><Filter className="h-3 w-3" /> Filter</button>
            <button className="flex items-center gap-1.5 hover:text-foreground transition-colors"><ArrowDownUp className="h-3 w-3" /> Sort</button>
            <div className="w-px h-3 bg-border mx-1" />
            <button className="flex items-center gap-1.5 hover:text-foreground transition-colors"><GroupIcon className="h-3 w-3" /> Group By: Status</button>
            
            <Button size="sm" className="ml-auto h-6 text-[10px] font-black tracking-widest px-4 rounded-sm bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm">
              NEW TASK
            </Button>
          </div>
      ...
      <div className={cn(
        ...
        isPropsOpen ? "w-[300px]" : "w-0 opacity-0 overflow-hidden pointer-events-none border-none"
      )}>
        ...
        <div className="w-full flex items-center p-2 gap-2 border-b border-border flex-shrink-0">
          <TabButton 
            ...
            onClick={() => { setActiveTab("info"); setIsPropsOpen(true); }} 
          />
          <TabButton 
            icon={MessageSquare} 
            active={activeTab === "comments"} 
            onClick={() => { setActiveTab("comments"); setIsPropsOpen(true); }} 
          />
          <TabButton 
            icon={Paperclip} 
            active={activeTab === "attachments"} 
            onClick={() => { setActiveTab("attachments"); setIsPropsOpen(true); }} 
          />
        ...
        <div className={cn(
          ...
          <div className="h-10 px-4 flex items-center justify-between border-b border-border">
            <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">{activeTab}</span>
          ...
          <ScrollArea className="flex-1">
            <div className="p-3 space-y-6">
               ...
               <div className="space-y-4">
                  <span className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest block border-b border-border pb-2">Properties</span>
                  
                  {/* Layer-Specific Props Holder */}
                  {layerType === EntityLayerType.ProjectSpace && <SpaceProps entityId={entityId} />}
                  {layerType === EntityLayerType.ProjectFolder && <FolderProps entityId={entityId} />}
                  {layerType === EntityLayerType.ProjectTask && <TaskProps entityId={entityId} />}
                  ...
                  <div className="space-y-4 pt-4 border-t border-border mt-4">
                    ...
                    <MetaRow label="ID" value={entityId.slice(0, 8).toUpperCase()} isMono />
                  </div>
               </div>

               <div className="pt-4">
                  <Button variant="outline" size="sm" className="w-full h-8 text-[10px] font-black tracking-widest border-border bg-muted/20 hover:bg-muted/40 uppercase">
                    Manage Access
                  </Button>
               </div>
            </div>
  ...
  return (
    <button 
      ...
      className={cn(
        ...
        active ? "text-primary bg-primary/10 shadow-[0_0_15px_rgba(var(--primary-rgb),0.1)]" : "text-muted-foreground/30 hover:text-muted-foreground/60 hover:bg-white/5"
      )}
    >
      <Icon className="h-3.5 w-3.5" />
      {active && <div className="absolute right-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-primary rounded-l-full shadow-lg" />}
    </button>
  );
}

function MetaRow({ label, value, color, isMono }: { label: string, value: string, color?: string, isMono?: boolean }) {
    return (
