...
export function TaskForm({ workspaceId, parentId, parentType, onSubmitSuccess, onCancel }: TaskFormProps) {
  ...
  const SelectedIcon = (LucideIcons as any)[icon] || CheckSquare
  ...
  return (
    <div className="flex flex-col w-[800px] bg-background overflow-hidden relative border-t border-border/50">
      <div className="flex flex-row h-[480px] w-full">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <Input 
                ...
                autoFocus
                onKeyDown={(e) => e.key === 'Enter' && name.trim() && createTask.mutate({ name, description, parentId, parentType }, { onSuccess: onSubmitSuccess })}
              />
            </div>
          </div>
        ...
        <div className="w-[280px] flex-shrink-0 border-l border-border bg-muted/5 p-6 flex flex-col gap-6 overflow-y-auto no-scrollbar">
           <div className="flex flex-col gap-1">
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground opacity-50">Task Details</span>
           </div>
           <div className="flex flex-col gap-4">
             ...
             <PropItem 
               ...
               onClick={() => setAssignee(assignee ? null : "Duc Nguyen")} 
             />
             <PropItem 
               label="Timeline" 
               icon={Calendar} 
               value={dueDate || "No Deadline"} 
               onClick={() => setDueDate(dueDate ? null : "Aug 24, 2026")} 
             />
           </div>
        </div>
      </div>
      ...
      <div className="px-6 py-4 bg-muted/10 border-t border-border/50 flex items-center justify-between flex-row">
          ...
          <Button 
              onClick={() => createTask.mutate({ name, description, parentId, parentType }, { onSuccess: onSubmitSuccess })} 
