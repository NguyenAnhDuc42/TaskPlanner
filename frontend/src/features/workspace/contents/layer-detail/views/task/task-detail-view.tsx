...
export function TaskDetailView() {
  ...
  const IconComponent = (Icons as any)[task.icon || ""] || Icons.CheckCircle2;
  const entityColor = task.color || "var(--primary)";

  // Resolve status and parent info
  const currentStatus = registry.statusMap[task.statusId || ""] || task.status;
        ...
        <div className="flex items-center gap-1.5 flex-wrap mt-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <div className="cursor-pointer">
                <PriorityBadge priority={task.priority} className="text-[10px] font-bold px-2.5 py-1" />
              </div>
            </DropdownMenuTrigger>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-muted/50 border border-border/10 text-[10px] font-bold text-muted-foreground hover:bg-muted transition-colors cursor-pointer">
                ...
                <span>{task.members?.length > 0 ? `${task.members.length} Users` : "No Members"}</span>
              </div>
            </PopoverTrigger>
