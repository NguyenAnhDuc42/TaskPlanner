    ...
    (statusId: string) => {
      setHiddenStatusIds((prev) => {
        ...
        const isIsolated =
          ...
          others.every((id) => prev.includes(id));
    ...
    <div className="px-2 py-2 flex items-center shrink-0 select-none gap-1 bg-background/20 backdrop-blur-sm">
          ...
          return (
            ...
            >
              <StatusBadge
                status={status}
                variant="outline"
                className="h-6 flex items-center pointer-events-none"
              />
              <button
      ...
      {onWorkflowOpen && (
        <button
          ...
          className="flex items-center justify-center h-6 w-6 rounded-full bg-muted/40 text-muted-foreground hover:bg-muted/80 hover:text-foreground border border-border/30 transition-all cursor-pointer"
      ...
      <div className="ml-auto flex items-center gap-1 shrink-0">
        <button className="ml-auto flex items-center h-6 gap-1 px-2 rounded-md bg-muted/40 text-[10px] text-muted-foreground font-semibold hover:bg-muted/80 hover:text-foreground border border-border/30 transition-all cursor-pointer shrink-0">
