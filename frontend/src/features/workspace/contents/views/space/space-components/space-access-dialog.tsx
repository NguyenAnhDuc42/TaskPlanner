...
export function SpaceAccessDialog({ spaceId, trigger }: SpaceAccessDialogProps) {
            ...
            <table className="w-full text-xs text-left">
              ...
              <tbody className="divide-y divide-border/10 text-muted-foreground/95">
                {allWorkspaceMembers.map((member) => {
                  ...
                  const handleLevelSelect = async (e: React.ChangeEvent<HTMLSelectElement>) => {
                    if (isCreator) return;
                    const newLevel = e.target.value as "Viewer" | "Editor" | "Manager" | "None";
                  ...
                  return (
                    <tr key={member.id || member.workspaceMemberId} className="hover:bg-white/[0.01] transition-colors">
                      ...
                      <td className="p-1 flex items-center gap-1.5">
                        <div className="h-4 w-4 rounded-full bg-primary/20 border border-border/20 flex items-center justify-center text-[8px] font-black text-white shrink-0">
                          {initials}
                        </div>
                        <div>
