...
export function TaskBoardView({
  ...
  // When not grouped by status, we render a single "All Tasks" column for now.
  // In a real app we might group by Priority or Assignee if we supported those.
  const columns = isGroupedByStatus
    ? statuses.map((s) => ({
        ...s,
        columnTasks: tasks.filter((t: TaskDto) => t.statusId === s.id),
      }))
