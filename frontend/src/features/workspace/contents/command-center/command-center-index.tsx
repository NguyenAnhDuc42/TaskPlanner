...
import { useCommandCenterStats } from "./command-center-api";
...
export default function CommandCenterIndex() {
  ...
  const { data: stats, isLoading } = useCommandCenterStats(workspaceId);

  if (isLoading) {
    return (
      <div className="flex-1 flex items-center justify-center font-mono text-xs uppercase tracking-widest text-muted-foreground animate-pulse">
  ...
  return (
    <div className="flex-1 p-6 space-y-6 overflow-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Active Projects" value={stats?.activeProjects.toString() || "0"} icon={LayoutDashboard} trend="+2 this week" />
        <StatCard title="Pending Tasks" value={stats?.pendingTasks.toString() || "0"} icon={Target} trend="-5% from yesterday" />
        <StatCard title="Velocity" value={stats?.velocity.toString() || "0"} icon={Zap} trend="Optimal" />
        <StatCard title="Health" value={stats?.health || "0%"} icon={Activity} trend="Stable" />
      </div>
