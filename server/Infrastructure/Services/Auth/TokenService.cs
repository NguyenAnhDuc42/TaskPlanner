...
public class TokenService : ITokenService
{
    ...
    { 
        ...
        var newRefreshExpiration = DateTimeOffset.UtcNow.AddDays(_jwtSettings.RefreshExpiration);
        
        // Rotate the token in session
        session.RotateToken(newRefreshToken);
        
        return new JwtTokens(newAccessToken, newRefreshToken, newExpiration, newRefreshExpiration);
    }

    public TimeSpan GetRefreshTokenDuration()
    {
        return TimeSpan.FromDays(_jwtSettings.RefreshExpiration);
    }
