using Microsoft.AspNetCore.Mvc;

namespace Api;

public static class UpdateSpaceEndpoint
{
    public static void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPut("/api/spaces/sync/{id:guid}", async (
            Guid id,
            [FromBody] UpdateSpaceCommand request,
            [FromHeader(Name = "X-Client-Trace-Id")] string? traceId,
            [FromServices] IHandler dispatcher,
            CancellationToken cancellationToken) =>
        {
            if (string.IsNullOrWhiteSpace(traceId))
            {
                return Results.BadRequest(new { Error = "X-Client-Trace-Id header is required for offline sync." });
            }

            request.SpaceId = id;
            request.TraceId = traceId;

            var result = await dispatcher.SendAsync<UpdateSpaceCommand, long>(request, cancellationToken);

            return result.IsSuccess
                ? Results.Ok(new { SyncEventId = result.Value })
                : Results.BadRequest(result.Error);
        })
        .RequireAuthorization()
        .WithTags("SpacesSync");
    }
}
