...
) : ICommandHandler<ForgotPasswordCommand, string?>
{
    public async Task<Result<string?>> Handle(ForgotPasswordCommand request, CancellationToken cancellationToken)
    {
        ...
        // The caller never learns whether the email actually sent (deliberately — we don't want
        // to leak account existence via a slow/failed-send response), so any SMTP failure here
        // (bad app-password, network, rejected auth — SmtpEmailService only guards the
        // unconfigured-host case) has to be caught and logged now, otherwise it's a silent black
        // hole and "reset password is broken" is undiagnosable from the server side.
        try
        {
