...
public static class DeleteDocumentBlockEndpoint
{
    public static void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapDelete("/api/document-blocks/sync/{id:guid}", async (
            Guid id,
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            var request = new DeleteDocumentBlockCommand { BlockId = id, TraceId = traceId };

            var result = await dispatcher.SendAsync<DeleteDocumentBlockCommand, long>(request, cancellationToken);
