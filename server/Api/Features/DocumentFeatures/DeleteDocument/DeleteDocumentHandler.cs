...
) : ICommandHandler<DeleteDocumentCommand, long>
{
    public async Task<Result<long>> Handle(DeleteDocumentCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = await db.ExecuteInTransactionAsync(async () =>
        {
            ...
            var syncPayload = JsonSerializer.Serialize(new { id = document.Id },
                SyncJson.Options);
            ...
            logger.LogInformation("Successfully deleted document {DocumentId} in database with SyncEvent", document.Id);
            return Result<long>.Success(0);
        }, cancellationToken);
