...
{
    public CreateViewHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService, WorkspaceContext workspaceContext) : base(unitOfWork, currentUserService, workspaceContext)
    {
    }

    public async Task<Guid> Handle(CreateViewCommand request, CancellationToken cancellationToken)
    {
        var view = ViewDefinition.Create(request.LayerId, request.LayerType, request.Name, request.ViewType,CurrentUserId);
