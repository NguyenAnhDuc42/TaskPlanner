...
public class ProjectFolderConfiguration : TenantEntityConfiguration<ProjectFolder>
{
    public override void Configure(EntityTypeBuilder<ProjectFolder> builder)
    {
        ...
        builder.Property(f => f.ProjectWorkspaceId).HasColumnName("project_workspace_id");
        builder.Property(f => f.ProjectSpaceId).HasColumnName("project_space_id");
        builder.Property(f => f.Name).HasColumnName("name").IsRequired().HasMaxLength(150);
        builder.Property(f => f.Slug).HasColumnName("slug").IsRequired().HasMaxLength(100);
        builder.HasIndex(f => new { f.ProjectSpaceId, f.Slug }).IsUnique();
        builder.Property(f => f.DefaultDocumentId).HasColumnName("default_document_id").IsRequired();
        builder.Property(f => f.OrderKey).HasColumnName("order_key").IsRequired();
        builder.Property(f => f.IsPrivate).HasColumnName("is_private");
        builder.Property(f => f.IsArchived).HasColumnName("is_archived");
        builder.Property(f => f.StartDate).HasColumnName("start_date");
        builder.Property(f => f.DueDate).HasColumnName("due_date");
        builder.Property(f => f.Color).HasColumnName("custom_color").HasMaxLength(16);
        builder.Property(f => f.Icon).HasColumnName("custom_icon").HasMaxLength(64);
