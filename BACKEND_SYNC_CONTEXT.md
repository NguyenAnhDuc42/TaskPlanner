...
**Frontend state found while researching this pass**: `WorkspaceMutations` (`workspace.mutations.ts`) already fully implements the create=backend-first / update=local-first-queued / delete=backend-first-no-queue split — nothing needed there beyond the backend fix above. `member.mutations.ts`/`status.mutations.ts` were empty stubs at the time this was written — **both built and wired into their UIs in a later pass this same session** (see §11a and `SYNC_SCENARIOS.md`). There is still no `entity-access.mutations.ts` file — `EntityAccess`/private-space checking is being phased out project-wide (see below), not worth the investment. All the relevant MobX stores + IndexedDB wrappers (`memberStore`/`memberDB`, `statusStore`/`statusDB`, `entityAccessStore`/`entityAccessDB`, plus `favoriteStore`/`favoriteDB` and `assigneeStore`/`assigneeDB` added later) are wired in `root.store.ts`.
...
## 11a. Favorites/Status/Member/Notification wired up for real, plus a Bootstrap versioning mechanism
This pass took every remaining backend-first entity (Favorite, Status, Member, Notification) from "endpoint exists, nothing calls it" to actually live in the app, and fixed real bugs along the way:

- **`GetBootstrapHandler`** now also returns `Favorites` (flat list, keyed by `entityId`+`entityLayerType`, no `entity_access` visibility filter — a favorite pointing at something the member can no longer see is just a dangling reference the frontend quietly skips) and `Members` (workspace-wide, also no visibility filter — every member can see every other member). Removed the `fav`/`entity_access`-joined `IsFavorite`/`FavoriteOrderKey` columns from the Task/Space/Folder queries entirely — favorites are no longer joined onto those records at all (see `FRONTEND_SYNC_CONTEXT.md` §1a for why that was wrong in the first place).
- **`SyncQueryService.CurrentDatabaseVersion`** bumped `1` → `2` (Members added to Bootstrap) and now actually *means* something — the frontend compares its last-bootstrap version against this and force-re-bootstraps if behind, instead of only bootstrapping on `lastSyncId === 0`. **Bump this every time Bootstrap's payload shape changes** (new entity type, new field, removed field) or existing sessions will never see the change — this is exactly what happened when Members were added and existing users saw an empty member list until this fix.
- **New `Api/Features/MemberFeatures/AddMembers`** (Command/Validator/Endpoint/Handler) — `POST /api/members/sync/add`. The existing `AddMembersHandler` (under `Application/Features/WorkspaceFeatures/MemberManage/AddMembers`) was legacy-pattern-in-new-clothing (no `SyncEvent`, still uses `NotifyEntitiesUpdatedAsync`) and still serves the old Redux UI — left untouched, a parallel sync-engine version was built instead. Returns `AddMembersResult(SyncEventId, List<MemberRecord> Members)` — richer than the usual `{ SyncEventId }` shape, because (a) the client can't generate a `WorkspaceMember` id up front (depends on resolving an email to an existing user server-side), and (b) `GroupExcept` means the adding client's own `SyncHub` connection never receives the resulting Delta anyway — the response body is the *only* way that client's local store ever learns about the members it just added.
- **Naming collision, 6th+ time**: `Api.AddMembersCommand` vs `Application.AddMembersCommand` broke `WorkspaceController.cs`'s `AddMembers` action — fully-qualified as `Application.AddMembersCommand` there (same fix pattern as every other entity in this migration).
- **Notification push gaps found in two Sync-engine handlers that otherwise fully replace their legacy counterparts**: `Api/Features/AssigneeFeatures/CreateAssignee/CreateAssigneeHandler` never pushed the `task_assigned` notification the legacy `UpdateTaskAssigneesHandler` did; `Api/Features/CommentFeatures/CreateComment/CreateCommentHandler` never pushed `comment_added`/`mention` notifications for assignees/task-creator/`@[memberId]`-mentions the legacy `AddCommentHandler` did. Both fixed by porting the same `NotificationService.PushAsync(...)` calls over, adapted to each handler's own field names. **Lesson: when a new Sync-engine handler "fully replaces" a legacy one, diff it against the legacy handler's side effects too, not just its core entity write** — SyncEvent/Delta plumbing is easy to remember to port, secondary notification pushes are easy to forget.
- **`FetchNotificationsEndpoint` had a real, pre-existing bug**: `[FromQuery] bool unreadOnly` was required with no default, so any request omitting it threw `BadHttpRequestException` → 500. Every real caller omits it in practice. Fixed by giving it a default and reordering params (`[FromServices]`/`CancellationToken` moved before the optional `[FromQuery]` params — required in C#, since optional parameters must trail; safe for minimal API since those two bind by type/attribute, not position).
- **`CreateTaskHandler`/`BatchFlushHandler`'s task-create path never derived a subtask's space/folder from its parent task** — it derived `effectiveSpaceId` from the Folder when one was given (the established "never trust the client for ancestor scope" pattern), but for `ParentTaskId` it just trusted whatever `ProjectSpaceId`/`ProjectFolderId` the client sent. Fixed: both paths now derive `effectiveSpaceId`/`effectiveFolderId` from the parent task's own values whenever `ParentTaskId` is set, overriding the client, same as the Folder case.
